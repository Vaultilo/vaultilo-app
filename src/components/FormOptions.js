...
export default function FormOptions(props) {
  ...
  const renderFormOptions = () => {
    return ITEM_TYPES.map(type => {
      return(
        <div className="col-4 wallet-box mb-3" key={`item__${type.value}`} onClick={() => setModalType(type.value)}>
          <Card>
            <Card.Body>
  ...
  return (
    <>
      { modalType !== 'items' ? (
        <div className="row">
          <button className="btn btn-link" onClick={() => setModalType('items')}>
            <i className="fa fa-arrow-left"></i> All Items
          </button>
        </div>) : null
      }
      {modalType === 'items' ?
        <div className="row">{renderFormOptions()}</div> : renderForm()}
    </>
  );
}
